...
| **The Notebook (M19, D47)** | Alan's own library of ideas and prompts — folders, search, autosave — as an overlay on Ctrl+Shift+N, over a working agent rather than instead of it. The part that makes it belong here: a note goes **into the prompt of the terminal on screen**, never submitted (§54's rule). Delivery is xterm's own `paste`, not a Rust command, because `session::typing::type_text` flattens newlines to spaces and would have shipped a multi-paragraph prompt as run-on lines while looking correct — measured against real Claude Code, not assumed. Sending is **refused** when the receiving program has not enabled bracketed paste, because that is twenty carriage returns and therefore twenty commands run. Deliberately not `project_notes` (§40): that is working memory about one project and is briefed; this belongs to no project and never is. |
| **Settings (§64, D45)** | Eight sections — Appearance, Terminal, Autonomy, Guardrails, Notifications, Phone, Environment, Updates — as a fixed map on the left with one section on the right, ordered by frequency of use. It was one long flat page until D45: everything was visible at once, which is not the same as findable. Guardrails is its own section because it grows with the core (§26); a section whose panel already titles itself gets no second heading from the shell. Which section is open lives in a module store so the palette can aim at one, and is never written to disk. Bounds are still enforced in the core rather than only drawn in the UI, and preferences apply in place so changing the terminal type size never rebuilds a running session |
